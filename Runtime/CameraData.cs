...
namespace LunarRenderPipeline {
    ...
    public struct CameraData {
        ...
        internal static GraphicsFormat MakeRenderTextureGraphicsFormat(bool isHdrEnabled, bool needsAlpha) {
            if (isHdrEnabled) {
        ...
        static RenderTextureDescriptor CreateRenderTextureDescriptor(Camera camera, float renderScale, bool isHdrEnabled, int msaaSamples, bool needsAlpha, bool requiresOpaqueTexture) {
            ...
            if (camera.targetTexture == null) {
                desc = new RenderTextureDescriptor(camera.pixelWidth, camera.pixelHeight) {
                    width = scaledWidth,
                    height = scaledHeight,
                    graphicsFormat = MakeRenderTextureGraphicsFormat(isHdrEnabled, needsAlpha),
                    depthBufferBits = 32,
        ...
        public static CameraData GetCameraData(LunarRenderPipelineAsset asset, Camera camera) {
            RenderTextureDescriptor descriptor = CreateRenderTextureDescriptor(camera, 1.0f, asset.supportsHDR, 1, Graphics.preserveFramebufferAlpha, false);
            bool anyShadowsEnabled = true/* settings.supportsMainLightShadows || settings.supportsAdditionalLightShadows */;
            
            float maxShadowDistance = Mathf.Min(10f/* asset.shadowDistance */, camera.farClipPlane);
            maxShadowDistance = (anyShadowsEnabled && maxShadowDistance >= camera.nearClipPlane) ? maxShadowDistance : 0.0f;
